...
def run_codesuture(script_path, cwd, timeout=30, extra_args=None):
    cmd = ["codesuture", "run"] + (extra_args or []) + [script_path]
    try:
...
def run_watch(script_path, cwd, extra_args=None, timeout=45):
    cmd = ["codesuture", "watch"] + (extra_args or []) + [script_path]
    try:
        r = subprocess.run(cmd, cwd=cwd, capture_output=True, text=True,
                           encoding="utf-8", errors="replace", timeout=timeout)
        return (r.stdout or "") + (r.stderr or ""), r.returncode
...
def check(label, output, must_contain=None, must_not_contain=None, section=None):
    ...
    reasons = []
    if output is None:
        results.append((label, False, ["codesuture not found on PATH"], section))
        print(f"  {FAIL}  {label}")
        return False
    if output == "TIMEOUT":
...
def clear_patches(cwd):
    for name in [".codesuture_cache", ".codesuture_knowledge",
                 ".codesuture_store", ".codesuture", "codesuture_patches",
...
def find_patch_store(cwd):
    for name in [".codesuture_cache", ".codesuture_store",
...
def run_section_A(workdir, project_path):
    ...
    r = subprocess.run(["codesuture", "--version"], capture_output=True, text=True,
                       encoding="utf-8", errors="replace")
    ok = r.returncode == 0 or "codesuture" in ((r.stdout or "") + (r.stderr or "")).lower()
    results.append(("codesuture on PATH", ok, [], "A"))
    print(f"  {PASS if ok else FAIL}  codesuture is on PATH")
    ...
    out, _ = run_codesuture("bug1_test.py", workdir)
    ...
    out, _ = run_codesuture("bug2_test.py", workdir)
    ...
    out2, _ = run_codesuture("bug2_test.py", workdir)
    ...
    out, _ = run_codesuture("bug3_hard_test.py", workdir)
    ...
    out, _ = run_codesuture("simple_reg_test.py", workdir)
...
def run_section_B(workdir):
    ...
    out, _ = run_codesuture("d1_test.py", workdir, extra_args=["--verbose"])
    ...
    out, _ = run_codesuture("d2_closure_test.py", workdir)
    ...
    out2, _ = run_codesuture("d2_closure_test.py", workdir)
    ...
    out, _ = run_codesuture("d3_shadow_test.py", workdir, extra_args=["--shadow"])
    check("--shadow accepted", out, must_not_contain=["unrecognized argument"], section="B-D3")
    check("Shadow warning fires", out, must_contain=["codesuture shadow"], section="B-D3")
    check("Mentions sentinel", out, must_contain=["sentinel"], section="B-D3")
    clear_patches(workdir)
    out_ns, _ = run_codesuture("d3_shadow_test.py", workdir)
    check("No shadow without --shadow", out_ns, must_not_contain=["codesuture shadow"], section="B-D3")

    header("D4 — TTL", "section B")
    write_test(os.path.join(workdir, "d4_test.py"), TEST_BUG1)
    clear_patches(workdir); run_codesuture("d4_test.py", workdir)
    ...
    if jfiles:
        ...
        if n > 0:
            ...
            out, _ = run_codesuture("d4_test.py", workdir)
    ...
    out1, _ = run_codesuture("d5_test.py", workdir)
    check("First run: no cache hit", out1, must_not_contain=["known crash pattern", "applying cached"], section="B-D5")
    fp = os.path.join(workdir, ".codesuture_fingerprints")
    check_bool(".codesuture_fingerprints created", os.path.isfile(fp), "not found", "B-D5")
    ...
    for name in [".codesuture_cache", ".codesuture_store", ".codesuture", "codesuture_patches"]:
        t = os.path.join(workdir, name)
        if os.path.isdir(t): shutil.rmtree(t)
        elif os.path.isfile(t): os.remove(t)
    out2, _ = run_codesuture("d5_test.py", workdir)
    ...
    clear_patches(workdir); run_codesuture("d6_test.py", workdir)
...
def run_section_C(workdir, project_path):
    ...
    clear_patches(workdir); run_codesuture("rollback_test.py", workdir)
    ...
    check_bool("Fingerprints removed",
               not os.path.isfile(os.path.join(workdir, ".codesuture_fingerprints")), "fp still exists", "C1")
    ...
    out, _ = run_codesuture("type_test.py", workdir)
    ...
    out2, _ = run_codesuture("type_test.py", workdir)
    ...
    out, _ = run_codesuture("index_test.py", workdir)
    ...
    out2, _ = run_codesuture("index_test.py", workdir)
    ...
    out, _ = run_codesuture("key_test.py", workdir)
    ...
    out2, _ = run_codesuture("key_test.py", workdir)
    ...
    out, _ = run_codesuture("dryrun_test.py", workdir, extra_args=["--dry-run"])
    ...
    check_bool("No fingerprints written",
               not os.path.isfile(os.path.join(workdir, ".codesuture_fingerprints")),
    ...
    check_bool("codesuture --version shows 0.5.0", "0.5.0" in (out or ""),
               f"got: {(out or '').strip()[:80]}", "C7")
    if project_path:
        r2 = subprocess.run(["python", "-c", "import codesuture; print(codesuture.__version__)"],
                            cwd=project_path, capture_output=True, text=True,
    ...
    for label, kw in [("Quick start","quick start"),("How it works","how it works"),
                       ...
                       ("Limitations","limitation"),("codesuture run","codesuture run"),
                       ("codesuture audit","codesuture audit"),("codesuture rollback","codesuture rollback"),
...
def run_section_D(workdir, project_path):
    ...
    print(f"  {DIM}Running: codesuture run async_test.py...{RESET}")
    out, _ = run_codesuture("async_test.py", workdir, timeout=30)
    ...
    out2, _ = run_codesuture("async_test.py", workdir, timeout=30)
    check("Async test: healed on second run", out2,
          must_contain=["already healed"], section="D1")

    header("D2 — Watch mode (codesuture watch)", "section D: v0.5")
    clear_patches(workdir)
    write_test(os.path.join(workdir, "watch_test.py"), TEST_WATCH)
    print(f"  {DIM}Running: codesuture watch watch_test.py --max-restarts 3...{RESET}")
    ...
    header("D3 — codesuture explain command", "section D: v0.5")
    clear_patches(workdir)
    write_test(os.path.join(workdir, "explain_setup.py"), TEST_EXPLAIN_SETUP)
    run_codesuture("explain_setup.py", workdir)  

    print(f"  {DIM}Running: codesuture explain...{RESET}")
    ...
    for fname, content, label, must_have, must_not in rw_tests:
        ...
        print(f"  {DIM}Running: codesuture run {fname}...{RESET}")
        out, _ = run_codesuture(fname, workdir, timeout=20)
        check(f"Real-world: {label}", out,
              must_contain=must_have, must_not_contain=must_not, section="D5")

        out2, _ = run_codesuture(fname, workdir, timeout=20)
    ...
    out, _ = run_cmd(["--version"], workdir)
    check_bool("codesuture --version shows 0.5.0",
               "0.5.0" in (out or ""),
               f"got: {(out or '').strip()[:80]}", "D6")
    if project_path:
        r2 = subprocess.run(
            ["python", "-c", "import codesuture; print(codesuture.__version__)"],
            cwd=project_path, capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=10)
        v = (r2.stdout or "").strip()
        check_bool("codesuture.__version__ == '0.5.0'", "0.5.0" in v, f"got: {v}", "D6")
...
def summary():
    ...
    if tf == 0:
        ...
        print("CodeSuture v0.5.0 — production-ready.")
    else:
        print(f"{BOLD}{RED}{tf} CHECK(S) FAILED{RESET} ({tp} passed, {ts} skipped / {total})")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CodeSuture verification suite v4.0")
    ...
    print(f"\n{BOLD}CodeSuture Verification Suite v4.0{RESET}")
