...
def run_section_B(workdir):
    ...
    fp_file = os.path.join(workdir, ".codesuture_fingerprints")
    check_bool(".codesuture_fingerprints created", os.path.isfile(fp_file),
               "file not found", "B-D5")
    ...
    header("D6 — codesuture audit command", "section B")
    write_test(os.path.join(workdir, "d6_test.py"), TEST_BUG3_FULL)
    clear_patches(workdir)
    run_codesuture("d6_test.py", workdir)
...
def run_section_C(workdir, project_path):
    ...
    print(f"  {DIM}Running: codesuture rollback --dry-run...{RESET}")
    ...
    print(f"  {DIM}Running: codesuture rollback --all...{RESET}")
    ...
    check_bool("rollback --all removes fingerprints",
               ...
               ".codesuture_fingerprints still exists", "C1")

    print(f"  {DIM}Running: codesuture rollback --all (empty store)...{RESET}")
    ...
    print(f"  {DIM}Running: codesuture rollback nonexistent_fn...{RESET}")
    ...
    print(f"  {DIM}Running: codesuture run type_test.py...{RESET}")
    ...
    print(f"  {DIM}Running: codesuture run index_test.py...{RESET}")
    ...
    print(f"  {DIM}Running: codesuture run key_test.py...{RESET}")
    ...
    header("C5 — codesuture run --dry-run mode", "section C: v0.4")
    clear_patches(workdir)
    write_test(os.path.join(workdir, "dryrun_test.py"), TEST_BUG3_FULL)
    print(f"  {DIM}Running: codesuture run --dry-run dryrun_test.py...{RESET}")
    ...
    check_bool("--dry-run does NOT write fingerprints",
               not os.path.isfile(os.path.join(workdir, ".codesuture_fingerprints")),
               ".codesuture_fingerprints written during dry-run", "C5")
    ...
    check_bool("codesuture --version shows 0.4.0",
               "0.4.0" in (out or ""),
    ...
    if project_path:
        ...
        check_bool("codesuture.__version__ == '0.4.0'",
    ...
    if not project_path:
        ...
        if not os.path.isfile(readme):
            ...
            for label, kw in [
                ...
                ("codesuture run",         "codesuture run"),
                ("codesuture audit",       "codesuture audit"),
                ("codesuture rollback",    "codesuture rollback"),
...
def summary():
    ...
    if tf == 0:
        ...
        print("CodeSuture v0.4.0 — engine + dark upgrades + release layer verified.")
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CodeSuture verification suite v3.0")
    ...
    print(f"\n{BOLD}CodeSuture Verification Suite v3.0{RESET}")
