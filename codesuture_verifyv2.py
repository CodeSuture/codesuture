...
def run_codesuture_cmd(args, cwd, timeout=15):
...
def check(label, output, must_contain=None, must_not_contain=None,
    ...
    if output is None:
        ...
        print(f"  {FAIL}  {label}")
        print(f"         {RED}codesuture not found{RESET}")
        return False
...
def test_install_check():
    ...
    result = subprocess.run(["codesuture", "--version"],
                            capture_output=True, text=True, encoding="utf-8", errors="replace")
    stdout = result.stdout or ""
    stderr = result.stderr or ""
    ok = result.returncode == 0 or         "codesuture" in (stdout + stderr).lower()
...
def test_d1_semantic_diff(workdir):
    ...
    output, _ = run_codesuture("d1_test.py", workdir, extra_args=["--verbose"])
...
def test_d2_caller_propagation(workdir):
    ...
    output, _ = run_codesuture("d2_closure_test.py", workdir)
    ...
    output2, _ = run_codesuture("d2_closure_test.py", workdir)
...
def test_d3_shadow_mode(workdir):
    ...
    print(f"  {DIM}Running: codesuture run --shadow d3_shadow_test.py...{RESET}")
    output, _ = run_codesuture("d3_shadow_test.py", workdir,
                              extra_args=["--shadow"])
    ...
    check("Shadow warning fires for sentinel return value", output,
          must_contain=["codesuture shadow"], section="B-D3")

    check("Shadow warning mentions sentinel value", output,
          must_contain=["sentinel"], section="B-D3")

    clear_patches(workdir)
    output_no_shadow, _ = run_codesuture("d3_shadow_test.py", workdir)
...
def test_d4_patch_ttl(workdir):
    ...
    if has_sidecars:
        ...
        if backdated > 0:
            ...
            output, _ = run_codesuture("d4_test.py", workdir)
...
def test_d5_fingerprint_registry(workdir):
    ...
    output1, _ = run_codesuture("d5_test.py", workdir)
    ...
    results.append((".codesuture_fingerprints file created after first run",
                    fp_exists, [] if fp_exists else
                    [".codesuture_fingerprints not found"], "B-D5"))
    print(f"  {PASS if fp_exists else FAIL}  "
          f".codesuture_fingerprints file created after first run")
    ...
    output2, _ = run_codesuture("d5_test.py", workdir)
...
def test_d6_audit_command(workdir):
    header("D6 — codesuture audit command",
           "section B: dark upgrades")
    ...
    print(f"  {DIM}Running: codesuture audit...{RESET}")
    output, rc = run_codesuture_cmd(["audit"], workdir)

    check("'codesuture audit' command runs without error", output,
    ...
    clear_patches(workdir)
    output_empty, _ = run_codesuture_cmd(["audit"], workdir)
    check("'codesuture audit' with no patches: clean message, no crash",
          output_empty,
...
def summary():
    ...
    if total_fail == 0:
        ...
        print(f"CodeSuture v0.3.0 — core + all 6 dark upgrades verified.")
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CodeSuture v2 verification suite — core + dark upgrades"
    )
    parser.add_argument("--path", "-p",
                        help="Path to codesuture project root (for pytest)",
                        default=None)
    ...
    print(f"\n{BOLD}CodeSuture Verification Suite v2.0{RESET}")
